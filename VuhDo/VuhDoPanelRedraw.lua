...
local function VUHDO_initPlayerTargetBorder(aButton, aBorderFrame)
	aBorderFrame:SetPoint("TOPLEFT", aButton:GetName(), "TOPLEFT",
		-VUHDO_INDICATOR_CONFIG["CUSTOM"]["BAR_BORDER"]["WIDTH"],
		VUHDO_INDICATOR_CONFIG["CUSTOM"]["BAR_BORDER"]["WIDTH"]);
	aBorderFrame:SetPoint("BOTTOMRIGHT", aButton:GetName(), "BOTTOMRIGHT",
		VUHDO_INDICATOR_CONFIG["CUSTOM"]["BAR_BORDER"]["WIDTH"],
		-VUHDO_INDICATOR_CONFIG["CUSTOM"]["BAR_BORDER"]["WIDTH"]);
...
local tIsVertical;
local tIsTurned;
local function VUHDO_positionAbsorbHintBar(aButton)
	tAbsHintBar = VUHDO_getHealthBar(aButton, 18);
	...
	tAbsHintBar:ClearAllPoints();
	if (tIsVertical) then
		tAbsHintBar:SetWidth(sBarScaling["barWidth"]);
		tAbsHintBar:SetHeight(3);
		if (tIsTurned) then
			tAbsHintBar:SetPoint("BOTTOMLEFT", VUHDO_getHealthBar(aButton, 3):GetName(), "BOTTOMLEFT", 0, 0);
		else
			tAbsHintBar:SetPoint("TOPLEFT", VUHDO_getHealthBar(aButton, 3):GetName(), "TOPLEFT", 0, 0);
		end
	else
		tAbsHintBar:SetWidth(3);
		tAbsHintBar:SetHeight(sBarHeight);
		if (tIsInverted) then
			tAbsHintBar:SetPoint("TOPLEFT", VUHDO_getHealthBar(aButton, 3):GetName(), "TOPLEFT", 0, 0);
		else
			tAbsHintBar:SetPoint("TOPRIGHT", VUHDO_getHealthBar(aButton, 3):GetName(), "TOPRIGHT", 0, 0);
		end
	end

	tAbsHintBar:SetFrameLevel(VUHDO_getHealthBar(aButton, 1):GetFrameLevel() + 2);
end
...
function VUHDO_positionHealButton(aButton)
	...
	tAbsBar = VUHDO_getHealthBar(aButton, 17);
	...
	VUHDO_positionAbsorbHintBar(aButton);
...
--
local function VUHDO_initAbsorbHintBar()
	local tAbsHintBar = VUHDO_getHealthBar(sButton, 18);
	local tHlBar = VUHDO_getHealthBar(sButton, 1);
	local tAbsorbConfig = VUHDO_INDICATOR_CONFIG["CUSTOM"]["ABSORB_BAR"];
	if (tAbsHintBar == nil) then
		return;
	end
	VUHDO_setLlcStatusBarTexture(tAbsHintBar, (tAbsorbConfig and tAbsorbConfig["TEXTURE"]) or "Clean");
	if (tHlBar ~= nil) then
		tAbsHintBar:SetFrameLevel(tHlBar:GetFrameLevel() + 2);
	end
...
local function VUHDO_initBarTexts(aButton, aHealthBar, aWidth)
	...
	tAddHeight = 0;
	if (not sLifeConfig["show"] or VUHDO_LT_POS_RIGHT == sLifeConfig["position"] or VUHDO_LT_POS_LEFT ==
		sLifeConfig["position"]) then
...
local function VUHDO_initSwiftmendIndicator()
	...
	tIcon:ClearAllPoints();
	tIcon:SetPoint("CENTER", sHealthBar:GetName(), "TOPLEFT", sBarScaling["barWidth"] / 5.5,
		-sBarScaling["barHeight"] / 14);
...
local function VUHDO_initTargetBar()
	if (sBarScaling["showTarget"]) then
		...
		VUHDO_getBarText(tTgHealthBar):SetTextColor(sTargetColor["TR"], sTargetColor["TG"], sTargetColor["TB"],
			sTargetColor["TO"]);
		VUHDO_getLifeText(tTgHealthBar):SetTextColor(sTargetColor["TR"], sTargetColor["TG"], sTargetColor["TB"],
			sTargetColor["TO"]);
...
--
local tHighlightBar;
local function VUHDO_initHighlightBar(aButton)
...
function VUHDO_initHealButton(aButton, aPanelNum)
	...
	VUHDO_getHealthBar(aButton, 18):SetIsInverted(tIsInverted);
	...
	VUHDO_getHealthBar(aButton, 18):SetOrientation(tOrientation);
...
local function VUHDO_initPanel(aPanel, aPanelNum)
	...
	aPanel:SetBackdropBorderColor(tPanelColor["BORDER"]["R"], tPanelColor["BORDER"]["G"], tPanelColor["BORDER"]["B"],
	...
	if (VUHDO_IS_PANEL_CONFIG) then
		...
		if (DESIGN_MISC_PANEL_NUM ~= nil and DESIGN_MISC_PANEL_NUM == aPanelNum and VuhDoNewOptionsPanelPanel ~= nil and
			...
			aPanel:SetBackdropBorderColor(tPanelColor["BORDER"]["R"], tPanelColor["BORDER"]["G"],
				tPanelColor["BORDER"]["B"], tPanelColor["BORDER"]["O"]);
		end
	...
	aPanel:SetBackdropColor(tPanelColor["BACK"]["R"], tPanelColor["BACK"]["G"], tPanelColor["BACK"]["B"],
		tPanelColor["BACK"]["O"]);
